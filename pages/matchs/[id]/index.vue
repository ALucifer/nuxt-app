    ...
    <div class="match-card--container">
      ...
      <div class="match-card--body">
        <div v-for="score in match.scores" :key="score.id" class="information">
          <p>Score indiqué par l'équipe : <b>{{ score.reporter.libelle }}</b></p>
          <p>
      ...
      <div v-if="!useHasScored()" class="match-card--footer">
        <button class="btn btn--score" @click.prevent="openModal">Donner son score</button>
      </div>
    </div>
    <Teleport to="body">
...
function useHasScored() {
  if (match.value.state === 'FINISH') {
    return true
  }

  const score = match.value.scores.find(score => score.reporter.userId === getUser().id)
