<template>
  <div
    v-if="isAfterNow(tournament.begin_at) && !isCompletlyClose(tournament)"
    class="time-area bg"
  >
    <div>
      <AppImage
        src="/images/waitng-icon.png"
        alt="image"
        class="tournament-card__wait"
      />
      <span>Début</span>
    </div>
  ...
  <div
    v-if="isCompletlyClose(tournament) && !isRunning(tournament)"
    class="date-area bg bg-danger"
  >
    Terminé
  </div>
  <div
    v-if="isRunning(tournament)"
    class="date-area bg bg-success"
  >
    En cours
  </div>
  <div
    v-if="isLogged() && isRegister(tournament)"
    class="date-area bg bg-warning"
  >
    Inscrit
  </div>
