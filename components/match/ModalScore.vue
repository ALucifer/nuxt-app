<template>
  <div id="myModal" ref="myModal" class="modal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content">
        <AppForm :validation-schema="schema" @submit="onSubmit">
          <div class="modal-header">
            <h5 class="modal-title">Renseigner un score</h5>
          ...
          <div class="modal-body d-flex gap-1 justify-content-evenly align-items-center">
            <div class="d-flex gap-3 align-items-center">
              <div class="d-flex flex-column align-items-center">
                ...
                <AppErrorMessage class="error" name="scoreA" />
              </div>
              <AppField name="scoreA" type="number" :validate-on-blur="false" :validate-on-change="false" />
            </div>
            <p>BO {{ match.tournament.bestOf }}</p>
            <div class="d-flex gap-3 align-items-center">
              <AppField name="scoreB" type="number" :validate-on-blur="false" :validate-on-change="false" />
              <div class="d-flex flex-column align-items-center">
                <AppAvatar :src="getAvatar(match.team_b)" />
                <p>{{ match.team_b.libelle }}</p>
                <AppErrorMessage class="error" name="scoreB" />
              </div>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
            <button class="btn btn--primary" @click="handleSubmit">Sauvegarder</button>
          </div>
        </AppForm>
      </div>
    </div>
